...
def test_not_ready_without_forward_clearance(env):
    """RTSM started without io.clearance.enable=true: pose and map are fine
    but the wall-guard signal is missing -> refuse (blind drive guard)."""
    cfg, rtsm, bridge, rtsm_srv, _ = env
    no_clearance = {k: v for k, v in GOOD_STATS.items() if k != "forward_clearance"}
    rtsm_srv.routes["/stats"] = no_clearance
    with TestClient(create_app(cfg, rtsm, bridge)) as c:
        s = c.get("/status").json()
        assert s["state"] == "NOT_READY"
        assert any("forward_clearance" in r for r in s["not_ready_reasons"])
        assert any("io.clearance.enable=true" in r for r in s["not_ready_reasons"])
        r = c.post("/command", json={"goal": "go to the red mug"})
        assert r.status_code == 503
