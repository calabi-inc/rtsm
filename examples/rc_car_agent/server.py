...
class AgentServer:
    ...
    def run_preflight(self) -> list:
        ...
        if not self.rtsm.healthz():
            reasons.append("RTSM unreachable — start it (python -m rtsm --set io.clearance.enable=true) in the GPU env")
        else:
            try:
                ...
                # nav's drive-phase obstacle guard is fail-open and the
                # relocation walk fail-closed, so a mission would run blind.
                if "forward_clearance" not in stats:
                    reasons.append(
                        "RTSM has no forward_clearance — restart it with "
                        "`python -m rtsm --set io.clearance.enable=true` "
                        "(drive guard + relocation walk need it)")
