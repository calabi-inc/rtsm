...
def _torch_load_with_compat(file, map_location='cpu'):
    """
    Call torch.load with weights_only=False for PyTorch >=2.6, and fall back if the argument
    is unsupported (older PyTorch versions).
    """
    try:
...
def torch_safe_load(weight):
    ...
    try:
        return _torch_load_with_compat(file, map_location='cpu'), file  # load
