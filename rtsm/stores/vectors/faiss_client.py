...
class FaissClient:
    ...
    def clear(self) -> Dict[str, int]:
        ...
        with self._lock:
            ...
            # A failed post-reset save is dangerous, not ignorable (PR #24):
            # the pre-reset vectors are still on disk, so the next process
            # restart resurrects them as ghost objects. Keep /reset non-fatal,
            # but say so loudly and record it so /healthz reports
            # vector_index_persist_failing until a save succeeds.
            if self.persistent_path:
                try:
                    self.save(self.persistent_path)
                except Exception as e:
                    msg = f"{type(e).__name__}: {e}"
                    logger.warning(
                        f"[FAISS] index save to {self.persistent_path} after clear "
                        f"failed: {msg} -- on-disk store still holds pre-reset "
                        f"vectors; a restart will resurrect them until a save succeeds"
                    )
                    self._last_save_error = msg
                else:
                    if self._last_save_error is not None:
