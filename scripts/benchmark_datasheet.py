...
def _repro_block() -> str:
    """Capture commit + key run config so the sheet is self-documenting and
    the x86 vs Jetson comparison is apples-to-apples."""
    try:
    ...
    try:
        ...
        seg, stg, asc, obj = c["segmentation"], c["staging"], c["assoc"], c["object"]
        cfg_lines = (
            ...
            f"- assoc: cos_min {asc['cos_min']}, gate_dist {asc['gate_dist_base_m']} m, "
            f"gate_reproj {asc['gate_reproj_px']} px\n"
            f"- object: promote_hits {obj['promote_hits']}, proto_ttl {obj['proto_ttl_s']} s"
        )
    except Exception as e:
        cfg_lines = f"(config read failed: {e})"
    return (
        f"## Reproducibility\n\n"
...
{_repro_block()}
> Caveat: N≈53-54 processed frames (camera gated ~20 Hz → ~5 Hz), real-time
> replay. Mean = steady-state; p95/max inflated by first-frame warmup.
